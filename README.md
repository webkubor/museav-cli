...
`studio-image` 是 [studio 出图中台](https://studio.webkubor.online) 的命令行客户端。装上它，配一个 apiKey，就能在终端里出图、逆向、图生图。
...
你永远不用接触上游 key，也不用关心用的哪个模型（除非你想指定）。**一个 key，就有出图能力。**

`studio-image` 这个 CLI 就是这套能力的命令行封装——本质和 `cs image gen` 是同一类东西：**一个账户一个 apiKey，拿着 key 调 API 就能出图。** 区别只是 studio 中台面向所有租户开放，任何人都能注册一个 key 来用。
...
- 做 AI 产品的开发者，想快速接入出图能力，不想碰上游细节
- 写自动化脚本 / agent 的人，需要命令行就能出图
- 有多个项目都需要出图，想统一收口到一个能力服务

**接入方式**：到中台注册一个租户，拿到 apiKey，装上这个 CLI（或直接调 API），就开始用。具体配额和计费见中台后台。
