...
This CLI is designed to be used directly by coding agents (Claude Code, Codex, etc.), not just humans. Read this before shelling out to `studio-image`.
...
- **You're acting as an individual user** (a person's own account): `studio-image login` — opens a device-authorization flow (prints a code + URL, polls until the user approves in a browser). Token cached in `~/.studio-image.json`, valid 7 days.
- **You're acting as a tenant/service** (no human in the loop, e.g. CI, a backend job): set `STUDIO_API_KEY=sk-studio-xxx` as an environment variable, or run `studio-image config --apiKey sk-studio-xxx`.
...
studio-image gen --prompt 'a poster, neon lights, cyberpunk' --ratio 9:16
